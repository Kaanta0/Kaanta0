...
def render_svg(profile: SteamProfile) -> str:
    ...
    badge_lines = "".join(
        f"<tspan x='16' dy='{0 if idx == 0 else 16}'>• {escape(badge_label(badge))}</tspan>"
        ...
        progress_rows.append(
            textwrap.dedent(
                ...
                <g transform="translate(0 {idx * 30})">
                  <text x="0" y="0" font-size="12" font-weight="600" fill="#E6F2FF">{escape(game.name)}</text>
                  <text x="{bar_width}" y="0" font-size="11" fill="#9EBFD8" text-anchor="end">{human_minutes(game.playtime_2weeks)}</text>
                  <rect x="0" y="10" width="{bar_width}" height="8" rx="4" fill="rgba(20, 38, 56, 0.85)" />
                  <rect x="0" y="10" width="{width:.1f}" height="8" rx="4" fill="url(#{gradient_id})" />
                </g>
                """
            ).strip()
        )
    ...
    return textwrap.dedent(
        ...
        <svg width="720" height="320" viewBox="0 0 720 320" fill="none" xmlns="http://www.w3.org/2000/svg">
          <defs>
            ...
            <filter id="steamCardShadow_{suffix}" x="-16%" y="-16%" width="132%" height="132%">
              <feDropShadow dx="0" dy="16" stdDeviation="22" flood-color="#040A14" flood-opacity="0.55" />
            </filter>
            <clipPath id="avatarClip_{suffix}">
              <rect x="32" y="44" width="128" height="128" rx="28" />
            </clipPath>
            {progress_defs}
          </defs>
          <g filter="url(#steamCardShadow_{suffix})">
            <rect x="0" y="0" width="720" height="320" rx="32" fill="url(#steamCardGradient_{suffix})" stroke="rgba(120, 194, 255, 0.28)" />
            <ellipse cx="96" cy="240" rx="96" ry="78" fill="url(#avatarGlow_{suffix})" />
          </g>
          <g font-family="'Segoe UI', 'Inter', 'Helvetica Neue', sans-serif">
            <image href="{avatar}" x="32" y="44" width="128" height="128" clip-path="url(#avatarClip_{suffix})" preserveAspectRatio="xMidYMid slice" />
            <rect x="32" y="44" width="128" height="128" rx="28" fill="rgba(11, 23, 35, 0.38)" stroke="rgba(115, 191, 255, 0.45)" />
            <circle cx="154" cy="156" r="12" fill="#0A141E" stroke="rgba(255,255,255,0.15)" />
            <circle cx="154" cy="156" r="8" fill="{status_color}" />
            <g transform="translate(188 82)">
              <text x="0" y="0" font-size="26" font-weight="700" fill="#F5FAFF">{escape(profile.personaname)}</text>
              <g transform="translate(0 16)">
                <rect x="0" y="14" width="112" height="28" rx="14" fill="rgba(16, 34, 52, 0.75)" stroke="rgba(108, 186, 255, 0.55)" />
                <text x="56" y="33" font-size="13" font-weight="600" fill="#D5ECFF" text-anchor="middle">Steam Level {escape(str(profile.level) if profile.level is not None else '??')}</text>
              </g>
              <text x="0" y="72" font-size="13" fill="#A7C3DB">{escape(info_line)}</text>
              <text x="0" y="94" font-size="13" fill="#7FB7DA">{escape(meta_line)}</text>
              <text x="0" y="116" font-size="13" fill="#66C0F4">{escape(activity_line)}</text>
            </g>
            <g transform="translate(32 204)">
              <rect x="0" y="0" width="148" height="108" rx="22" fill="rgba(16, 31, 46, 0.62)" stroke="rgba(120, 194, 255, 0.28)" />
              <text x="18" y="30" font-size="13" font-weight="600" fill="#7FCBFF">Featured badges</text>
              <text x="18" y="54" font-size="12" fill="#C7E7FF">{badge_lines}</text>
            </g>
            <g transform="translate(188 152)">
              <rect width="480" height="150" rx="26" fill="rgba(12, 26, 40, 0.78)" stroke="rgba(120, 194, 255, 0.25)" />
              <text x="24" y="36" font-size="14" font-weight="600" fill="#7FCBFF">Recent playtime</text>
              <g transform="translate(24 66)">
                {progress_section}
              </g>
            </g>
            <text x="188" y="300" font-size="12" fill="rgba(199, 231, 255, 0.65)">Updated {escape(updated_label)}</text>
            <a href="{escape(profile.profileurl)}" target="_blank" rel="noreferrer">
              <rect x="596" y="36" width="92" height="36" rx="14" fill="rgba(18, 42, 60, 0.75)" stroke="rgba(120, 194, 255, 0.4)" />
              <text x="642" y="59" font-size="13" font-weight="600" fill="#F5FAFF" text-anchor="middle">View Profile</text>
            </a>
          </g>
        </svg>
        """
