...
def render_svg(profile: SteamProfile) -> str:
    ...
    avatar = escape(profile.avatar_data_uri or profile.avatarfull or DEFAULT_AVATAR_DATA_URI)
    level_text = f"Level {profile.level}" if profile.level is not None else "Level hidden"

    recent_height = 56 + 16 * max(0, len(recent) - 1)
    badge_height = 56 + 16 * max(0, len(badges) - 1)
    header_block_height = 120
    card_gap = 12
    badge_block_y = header_block_height + recent_height + card_gap
    total_height = badge_block_y + badge_height + 24

    return textwrap.dedent(
        f"""
        <svg width="360" height="{total_height}" viewBox="0 0 360 {total_height}" fill="none" xmlns="http://www.w3.org/2000/svg">
          <defs>
          ...
          <rect x="24" y="26" width="88" height="88" rx="18" fill="rgba(15, 29, 44, 0.55)" stroke="rgba(102,192,244,0.45)" />
          ...
          <g transform="translate(24 {header_block_height})" font-family="'Segoe UI', 'Inter', sans-serif">
            <rect width="312" height="{recent_height}" rx="16" fill="rgba(15, 29, 44, 0.85)" stroke="rgba(102,192,244,0.35)" />
            <text x="20" y="26" font-size="13" font-weight="600" fill="#66C0F4">Recent playtime</text>
            <text x="20" y="42" font-size="12" fill="#B5D8F2">
              <tspan x="20" dy="0">{escape(recent[0].name)} — {human_minutes(recent[0].playtime_2weeks)}</tspan>
              {recent_lines}
            </text>
          </g>
          <g transform="translate(24 {badge_block_y})" font-family="'Segoe UI', 'Inter', sans-serif">
            <rect width="312" height="{badge_height}" rx="16" fill="rgba(12, 24, 36, 0.85)" stroke="rgba(102,192,244,0.35)" />
            <text x="20" y="26" font-size="13" font-weight="600" fill="#66C0F4">Badge highlights</text>
            <text x="20" y="42" font-size="12" fill="#B5D8F2">
          ...
          <a href="{escape(profile.profileurl)}" target="_blank" rel="noreferrer">
            <rect x="260" y="30" width="76" height="30" rx="10" fill="rgba(18, 42, 60, 0.85)" stroke="rgba(102,192,244,0.45)" />
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    if not profile.avatar_data_uri:
        fetched_avatar = fetch_avatar_data(session, profile.avatarfull)
        if fetched_avatar:
            profile.avatar_data_uri = fetched_avatar
